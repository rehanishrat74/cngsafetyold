...
class ApiController extends Controller
{
    ...
    public function uploadFiles(Request $r) {
        ...
        $response=array('response'=>$finalResponse,'msg'=>$msg,'trace'=>$trace);
    ...
    public function doLogin(Request $r) {
	   ...
	   $user_code = $r['email'];
        ...
        if(Auth::attempt($cred)) {
            ...
            $response['user_name'] = Auth::user()->user_name;
    ...
    public function doVerifyCode(Request $r) {
	 ...
	 $stationno = $r['stationno']; //Additional Field
	 $scan_code = $r['scan_code']; //Additional Field
     ...
     if ($dbsticker[0]->validShopSticker==0){
        $responsecode = "invalid";
        $responsemsg="Invalid sticker for this shop.";
     }
     else if ($dbsticker[0]->validShopSticker==1)
     {  
    ...
    {
        $response = array();
        $scan_code = $r['scan_code']; //Addoitional field        
    ...
    public function doUpdateCylinders(Request $r) {
        ...
        $userid = $r['user_id'];
        $code = $r['code'];
	    $scan_code = $r['scan_code']; //Additional fields
